...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, x, w):
        ...
        weight = weight * demod
        weight = weight.view(B * self.out_channels, self.in_channels, *self.conv.kernel_size)
...
class SynthesisBlock(nn.Module):
    ...
    def forward(self, x, w, noise1=None, noise2=None):
        ...
        noise1 = noise1 or torch.randn(B, self.conv1.out_channels, H, W, device=x.device)
        x = self.conv1(x, w) + self.noise_scale1 * noise1
        x = F.leaky_relu(x, 0.2)
        # Generate noise2 with out_channels (same as conv2 output)
        noise2 = noise2 or torch.randn(B, self.conv2.out_channels, H, W, device=x.device)
...
class Generator(nn.Module):
    ...
    def forward(self, z, truncation_psi=1.0):
        w = self.mapping(z)
        if self.training:
            self.w_avg.copy_(self.truncation_beta * self.w_avg + (1 - self.truncation_beta) * w.mean(0))
